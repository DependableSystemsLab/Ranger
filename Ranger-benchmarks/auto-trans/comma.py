...
tx, ty = driving_data.LoadTestSet()
...
up_bound = [2., 3., 1., 1.] # upper bound for restriction
low_bound = [-1.,-1.,-1.,-1.] # low bound for restriction
...
preds = OLD_SESS.run(OP_FOR_EVAL, feed_dict={model.x: [image], model.prob1: 1., model.prob2: 1.})
print(preds)
print('')
...
#new_y = sess.graph.get_tensor_by_name( get_op_with_prefix(y.op.name, graph_dup_cnt, PREFIX)+":0")
new_prob1 = sess.graph.get_tensor_by_name( get_op_with_prefix(model.prob1.op.name, graph_dup_cnt, PREFIX, dummy_graph_dup_cnt, DUMMY_PREFIX)+":0")
...
preds = sess.run(new_op_for_eval, feed_dict={new_x: [image], new_prob1: 1., new_prob2: 1.})
print(preds)
