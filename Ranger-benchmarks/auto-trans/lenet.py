...
#    tanh = tf.math.minimum(tanh, tf.constant(3.)) 
...
#    pool = tf.math.minimum(pool, tf.constant(3.))
...
#    tanh = tf.math.minimum(tanh, tf.constant(4.)) 
...
#    pool = tf.math.minimum(pool, tf.constant(4.))
...
#    reshape = tf.math.minimum(reshape, tf.constant(4.))


    hidden = tf.nn.relu(tf.matmul(reshape, fc1_weights) + fc1_biases)


#    tanh = tf.math.minimum(tanh, tf.constant(8.)) 
...
#    checkpointer.restore(sess, save_path=_SAVE_PATH_)
  ...
  # eval on the original graph   
  a = OLD_SESS.run( OP_FOR_EVAL, feed_dict={eval_data: test_data[0:1, ...]} )
  print( a.flatten()[:10] )
  print("=======================")

  # eval on the new graph with bounding ops
  newinput = get_op_with_prefix(eval_data.op.name, graph_dup_cnt, PREFIX, dummy_graph_dup_cnt, DUMMY_PREFIX)
  b = sess.run(new_op_for_eval, feed_dict={sess.graph.get_tensor_by_name(newinput+":0"): test_data[0:1, ...]}) 
  print( b.flatten()[:10] )
