...
np.random.seed(26) # set random seed for reproducibility purpose
...
def bitElementTensor( dtype, val):
    ...
    prevIndex = {} # mark the index of injected fault and injected value
    numOfCurrFault = 1
    while( numOfCurrFault>0):


        index = np.random.randint(low=0, high=len(val))

        # keep track of the value before last injection (init to be the original value)
        bfVal = val[index]
        injectedVal = randomBitFlip(val[index])


        # keep track of all the faulty value injected (in case multi-fault into same data, it might offset, e.g., bit-flips at the same location). This is not likely to happen for concurrent faults, should be on different locations 
        if(not ( index in prevIndex and injectedVal in prevIndex[index])):
            numOfCurrFault -= 1
            val[index] = injectedVal

            # this value has been injected before (but not on the same bit)
            if( index in prevIndex):    
                prevIndex[ index ].append( bfVal )
            else:
                prevIndex[ index ] = []
                prevIndex[ index ].append( bfVal )
