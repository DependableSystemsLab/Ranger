        ...
        if (count > fiConf.skipCount):  
            ...
            if(fiConf.injectMode == "oneFaultPerRun"):
                ...
                if(injectedOp == 0):
                    ...
                    "here we control the range of ops to be injected (e.g., if you want to exclude the last FC that has 2 ops, set the high=totalInstance-1)"
...
def injectFaultAtan(a):
        "Function to call injectFault on Atan"
        logging.debug("Calling Operator ATAN " + getArgs(a))

        atan = tf.atan(a)
        with tf.Session() as sess:
                res = atan.eval()
        res = condPerturb(Ops.ATAN, res)
        if logReturn: logging.debug("\tReturning from ATAN " + str(res) )
        return res
...
def injectFusedBatchNorm(a, gamma, beta, moving_mean, moving_variance):
    ...
    '''
    resOp = tf.layers.batch_normalization( a  )
    with tf.Session() as sess:
        print("=====")
        res = resOp.eval() 
    '''
