	...
	if fiConf.isSelected(op): 
		...
		if (count > fiConf.skipCount):	
			...
			if(fiConf.injectMode == "oneFaultPerRun"):
				...
				# we only consider the Ops that are actually doing data computation,
				# the dim of the data is greater than 1 (this is a specific case in the ResNet model)
				if( res.ndim >1 ):
				...
				if(injectedOp == 0):
					...
					############# We don't inject in the last 3 Ops AFTER ReLu
					injectedOp = np.random.randint(low= 1 , high= 81) 
					########
...
def injectFaultRelu(a):
	...
	global reluNum 
	upperBound = [7, 8, 7, 5, 11, 5, 12, 6, 11, 5, 12, 5, 14, 5, 12, 5, 66]
...
def injectFaultConcatV2(a, b, c):
	...
	op = tf.concat(a,b)
	res = sess.run(op)
	return res #injectFaultPack(a, b)
...
def injectFusedBatchNorm(a, gamma, beta, moving_mean, moving_variance):
	# This might not be a right implementation
	res = a
