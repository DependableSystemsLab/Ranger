	...
	if fiConf.isSelected(op): 
		...
		if (count > fiConf.skipCount):	
			...
			if(fiConf.injectMode == "errorRate" ):
				...
				if(op.name=="IDENTITY"): print(op.name)
...
def injectFaultReshape(a, b):
	...
	global reluNum 
	upperBound = [956, 5118, 9870, 16892, 27154, 20696, 25586, 19959, 9975, 5791, 4118, 2379, 869, 145, 42]
	res = np.minimum(res, upperBound[reluNum-1], res)
...
def injectFaultRelu(a):
	...
	global reluNum 
	upperBound = [956, 5118, 9870, 16892, 27154, 20696, 25586, 19959, 9975, 5791, 4118, 2379, 869, 145, 42]
	res = np.minimum(res, upperBound[reluNum], res) # upper bound rest.
	res = np.maximum(res, 0, res)	# lower bound rest.
...
def injectFaultMaxPool(a, ksize, strides, padding): 
	...
	global reluNum 
	upperBound = [956, 5118, 9870, 16892, 27154, 20696, 25586, 19959, 9975, 5791, 4118, 2379, 869, 145, 42]
	res = np.minimum(res, upperBound[reluNum-1], res) # upper bound rest.
	res = np.maximum(res, 0, res)	# lower bound rest. 
	"=============================================="
...
def injectFaultBiasAdd(a, b):
	...
	#b = condPerturb(Ops.BIASADD, b)

	sess = tf.Session()
	op = tf.nn.bias_add(a,b)
	res = sess.run(op)
